...
from src.retrieval import VectorRetriever, HybridRetriever, format_page_display
...
class QuestionsProcessor:
    def __init__(
        ...
        llm_reranking: bool = False,              # 是否启用重排
        llm_reranking_sample_size: int = 5,
        rerank_model: str = "dashscope",
        rerank_api_key: str | None = None,
        top_n_retrieval: int = 10,
        parallel_requests: int = 10,
        api_provider: str = "dashscope", # openai
        answering_model: str = "qwen3.7-plus", # gpt-4o-2024-08-06
        ...
        self._last_retrieval_elapsed = 0.0
        self._last_rerank_elapsed = 0.0
        self._last_rerank_fallback = False
        self._last_rerank_fallback_reason: str | None = None
        self._last_llm_elapsed = 0.0
        self._last_retrieval_results = []
        self._last_stream_answer = None
    ...
    def _format_retrieval_results(self, retrieval_results) -> str:
        """将检索结果格式化为带 [N] 编号的 RAG 上下文字符串"""
        if not retrieval_results:
            return ""

        context_parts = [
            "以下检索片段已编号为 [1]、[2]…，请仅基于这些片段作答；"
            "若均不足以回答问题，请回答「未在检索资料中找到相关信息」。",
        ]
        for i, result in enumerate(retrieval_results, 1):
            result["chunk_index"] = i
            page_number = result.get("page")
            page_label = format_page_display(page_number)
            if page_label == "-":
                page_label = "unknown"
            source_name = (
            ...
            if str(source_name).lower().endswith(".md"):
                source_name = str(source_name)[:-3]
            text = result["text"]
            context_parts.append(
                f'[{i}] 来源: {source_name}, 页码: {page_label}\n"""\n{text}\n"""'
            )
    ...
    def _apply_citation_verification(
        ...
        """用后端校验覆盖 LLM 生成的引用元数据（含全语料反向查找）。"""
        from src.citation_verification import verify_citations
        ...
        cited = verified.get("cited_retrieval_results") or []
        self._last_cited_retrieval_results = cited
        self._last_original_retrieval_results = list(retrieval_results)
        return verified
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        self._record_retrieval_timing(retriever)
        ...
        self._last_llm_elapsed = t6 - t5
        self.response_data = self.openai_processor.response_data
        if self.new_challenge_pipeline:
            answer_dict = self._apply_citation_verification(
                answer_dict, retrieval_results, company_name=company_name
            )
        print(f"[计时] [get_answer_for_company] 总耗时: {t6-t0:.2f} 秒")
        return answer_dict

    def get_answer_for_company_stream(self, company_name: str, question: str, schema: str):
        """流式生成答案，逐块 yield LLM 文本；结束后调用 get_last_stream_answer 获取结构化结果。"""
        t0 = time.time()
        if self.llm_reranking:
            retriever = HybridRetriever(
                vector_db_dir=self.vector_db_dir,
        ...
        t_llm = time.time()
        for chunk in self.openai_processor.get_answer_from_rag_context_stream(
            question=question,
            rag_context=rag_context,
            schema=schema,
            model=self.answering_model,
        ...
        self._last_llm_elapsed = time.time() - t_llm
        answer_dict = self.openai_processor.get_last_stream_answer() or {}
        self.response_data = self.openai_processor.response_data
        if self.new_challenge_pipeline:
            answer_dict = self._apply_citation_verification(
                answer_dict, retrieval_results, company_name=company_name
            )
        self._last_stream_answer = answer_dict
        print(f"[计时] [get_answer_for_company_stream] 总耗时: {time.time()-t0:.2f} 秒")

    def get_last_stream_answer(self) -> Optional[dict]:
        return getattr(self, "_last_stream_answer", None)

    def get_last_retrieval_results(self) -> list:
        cited = getattr(self, "_last_cited_retrieval_results", None)
        if cited:
            return cited
        return getattr(self, "_last_retrieval_results", [])

    def get_last_original_retrieval_results(self) -> list:
        return getattr(self, "_last_original_retrieval_results", None) or getattr(
            self, "_last_retrieval_results", []
        )

    def get_last_cited_retrieval_results(self) -> list:
        return getattr(self, "_last_cited_retrieval_results", [])

    def get_last_retrieval_elapsed(self) -> float:
        """仅向量库 faiss.search 耗时（不含 embedding API 与 Jina 重排）。"""
        return getattr(self, "_last_retrieval_elapsed", 0.0)

    def get_last_rerank_elapsed(self) -> float:
        return getattr(self, "_last_rerank_elapsed", 0.0)

    def get_last_rerank_fallback(self) -> bool:
        return getattr(self, "_last_rerank_fallback", False)

    def get_last_rerank_fallback_reason(self) -> str | None:
        return getattr(self, "_last_rerank_fallback_reason", None)

    def get_last_llm_elapsed(self) -> float:
        return getattr(self, "_last_llm_elapsed", 0.0)

    def _record_retrieval_timing(self, retriever) -> None:
        self._last_retrieval_elapsed = getattr(retriever, "last_vector_db_elapsed", 0.0)
        self._last_rerank_elapsed = getattr(retriever, "last_rerank_elapsed", 0.0)
        self._last_rerank_fallback = getattr(retriever, "last_rerank_fallback", False)
        self._last_rerank_fallback_reason = getattr(retriever, "last_rerank_fallback_reason", None)
    ...
    def process_single_question_stream(self, question: str, kind: str = "string"):
        """单条问题流式推理，yield LLM 文本块；结束后调用 get_last_stream_answer。"""
        if self.new_challenge_pipeline:
            extracted_companies = self._extract_companies_from_subset(question)
        else:
            extracted_companies = re.findall(r'"([^"]*)"', question)
        ...
        if len(extracted_companies) == 0:
            raise ValueError("No company name found in the question.")
        if len(extracted_companies) > 1:
            raise ValueError("流式输出暂不支持多公司比较问题，请使用单公司问题。")

        company_name = extracted_companies[0]
        yield from self.get_answer_for_company_stream(
            company_name=company_name,
            question=question,
            schema=kind,
        )

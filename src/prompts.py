...
class AnswerWithRAGContextSharedPrompt:
    instruction = """
你是一个RAG（检索增强生成）问答系统。
你的任务是**仅基于**下方以 [1]、[2]… 编号的检索片段回答给定问题。

严格规则（必须遵守）：
- 只能使用上下文中明确出现的信息；禁止使用自身知识、训练语料或外部常识补充。
- 上下文中的每个片段以 [N] 编号标注；只能引用这些编号对应的片段内容。
- 若所有片段均不足以回答问题，final_answer 必须且只能回答：「未在检索资料中找到相关信息」。
- 不要引用上下文中不存在的页码、文件名、文档标题或数据。
- 引用元数据（页码、文件名）由系统自动映射，你无需也不应自行填写。
...
class AnswerWithRAGContextStringPrompt:
    instruction = AnswerWithRAGContextSharedPrompt.instruction + """
开放性文本回答的引用规则（必须遵守）：
- final_answer 中凡引用检索片段的内容，应在相应语句末尾标注 [N] 角标（如 [1]）；若无法基于片段作答则不要标注。
- 仅使用 [N] 角标指代片段，不要写文件名、页码或文档标题。
- source_chunk_indices 填写你实际参考的片段编号（1-based，对应上下文 [N]）；若无可用片段则返回空列表 []。
- 角标与 source_chunk_indices 由后端二次校验；你只需如实标注实际引用的片段。
- 若上下文不足以回答，final_answer 使用「未在检索资料中找到相关信息」，source_chunk_indices 为 []。
"""
    user_prompt = AnswerWithRAGContextSharedPrompt.user_prompt

    class AnswerSchema(BaseModel):
        final_answer: str = Field(description="""
最终答案为一段完整、连贯的文本，需严格基于 [N] 编号片段作答。
若上下文无相关信息，必须回答「未在检索资料中找到相关信息」，不要编造内容。
请优先输出此字段，便于流式展示。
引用检索内容时，在句末标注 [1]、[2] 等角标；不要写文件名或页码。
""")
        step_by_step_analysis: str = Field(description="""
详细分步推理过程，至少5步，150字以上。请结合上下文信息，逐步分析并归纳答案。
引用片段时使用 [N] 角标，不要写页码或文件名。
""")
        reasoning_summary: str = Field(description="简要总结分步推理过程，约50字。")
        source_chunk_indices: List[int] = Field(description="""
实际用于回答问题的检索片段编号列表（1-based，对应上下文中的 [1]、[2] 等）。
仅填写直接支撑 final_answer 的片段编号；不要编造不存在的编号。
若 final_answer 为「未在检索资料中找到相关信息」，必须返回空列表 []。
""")
...
```
{
  "final_answer": "万科企业股份有限公司2022年主营业务包括房地产开发、物业服务、租赁住房、物流仓储等，核心业务为住宅及商业地产开发与运营。[1][2]",
  "step_by_step_analysis": "1. 问题要求总结2022年万科企业股份有限公司的主营业务。\n2. [1] 与 [2] 详细描述了公司主营业务，包括房地产开发、物业服务等。\n3. 结合上下文，归纳出主要业务板块。\n4. 重点突出房地产开发和相关服务。\n5. 形成简明扼要的总结。",
  "reasoning_summary": "[1][2] 明确列出主营业务，答案基于原文归纳。",
...
```
{
  "final_answer": "未在检索资料中找到相关信息",
  "step_by_step_analysis": "1. 问题要求…\n2. 逐一检查 [1]、[2] 片段…\n3. 片段中未包含所需信息…\n4. 不能凭常识补充…\n5. 结论：资料不足。",
  "reasoning_summary": "检索片段未覆盖问题要点，无法作答。",

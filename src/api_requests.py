...
from src.model_config import (
    ...
    dashscope_disable_thinking_for_rag,
    dashscope_uses_multimodal_api,
)

# RAG 回答默认参数：低温度保证稳定，限制输出长度加速生成
RAG_ANSWER_TEMPERATURE = 0.1
RAG_ANSWER_MAX_TOKENS = 1024


def _to_multimodal_messages(messages: List[Dict]) -> List[Dict]:
    mm_messages = []
    for msg in messages:
        content = msg["content"]
        if isinstance(content, str):
            mm_messages.append({"role": msg["role"], "content": [{"text": content}]})
        else:
            mm_messages.append(msg)
    return mm_messages


def dashscope_chat_call(
    model: str,
    messages: List[Dict],
    temperature: float = 0,
    ...
    if stream:
        ...
        common_kwargs["incremental_output"] = incremental_output

    if dashscope_uses_multimodal_api(model):
        if enable_thinking is None and dashscope_disable_thinking_for_rag(model):
            enable_thinking = False
        if enable_thinking is not None:
            common_kwargs["enable_thinking"] = enable_thinking
...
def extract_dashscope_stream_delta(rsp) -> str:
    """从 DashScope 流式响应块中提取增量文本。"""
    status_code = rsp.get("status_code") if isinstance(rsp, dict) else getattr(rsp, "status_code", None)
    if status_code and status_code != 200:
        code = rsp.get("code") if isinstance(rsp, dict) else getattr(rsp, "code", "")
        message = rsp.get("message") if isinstance(rsp, dict) else getattr(rsp, "message", "")
        raise RuntimeError(f"DashScope stream error ({status_code}, {code}): {message}")

    output = rsp.get("output") if isinstance(rsp, dict) else getattr(rsp, "output", None)
    if not output:
        return ""
...
def extract_dashscope_message_content(rsp) -> str:
    status_code = rsp.get("status_code") if isinstance(rsp, dict) else getattr(rsp, "status_code", None)
    if status_code and status_code != 200:
        code = rsp.get("code") if isinstance(rsp, dict) else getattr(rsp, "code", "")
        message = rsp.get("message") if isinstance(rsp, dict) else getattr(rsp, "message", "")
        raise RuntimeError(f"DashScope API error ({status_code}, {code}): {message}")

    output = rsp.get("output") if isinstance(rsp, dict) else getattr(rsp, "output", None)
    if not output:
        raise RuntimeError(f"DashScope返回无 output: {rsp}")
...
class APIProcessor:
    ...
    def get_answer_from_rag_context(self, question, rag_context, schema, model):
        ...
        return self._normalize_rag_answer(answer_dict)

    _RAG_ANSWER_EXTRA_FIELDS = (
        ...
        "answer_grounded",
        "citation_source",
        "cited_retrieval_results",
        "original_retrieval_results",
        "unverified_sentences",
        "citation_matches",
        "reverse_citation_matches",
    )

    def _normalize_rag_answer(self, answer_dict) -> dict:
        """统一 RAG 答案字段结构，并保留引用校验相关字段。"""
        ...
        if "step_by_step_analysis" not in answer_dict:
            if "final_answer" in answer_dict:
                ...
                if isinstance(final_answer_content, str) and final_answer_content.strip().startswith("{"):
                    try:
                        return self._normalize_rag_answer(json.loads(final_answer_content))
                ...
                normalized = {
                    ...
                    "source_chunk_indices": answer_dict.get("source_chunk_indices", []),
                }
            else:
                normalized = {
                    "step_by_step_analysis": "",
            ...
            normalized = dict(answer_dict)
            normalized.setdefault("source_chunk_indices", [])

        for key in self._RAG_ANSWER_EXTRA_FIELDS:
            if key in answer_dict and answer_dict[key] is not None:
                normalized[key] = answer_dict[key]
        return normalized

    def get_answer_from_rag_context_stream(self, question, rag_context, schema, model):
        """流式生成 RAG 答案，逐块 yield 文本；结束后可通过 get_last_stream_answer 取结构化结果。"""
        system_prompt, response_format, user_prompt = self._build_rag_context_prompts(schema)
        user_content = user_prompt.format(context=rag_context, question=question)
        self._last_stream_answer = None

        if self.provider == "dashscope":
            for chunk in self.processor.send_message_stream(
                model=model,
                system_content=system_prompt,
                ...
                yield chunk
            self.response_data = self.processor.response_data
            answer_dict = self.processor.last_parsed_response
        else:
            answer_dict = self.get_answer_from_rag_context(
                question=question,
                rag_context=rag_context,
                schema=schema,
                model=model,
            )
            text = json.dumps(answer_dict, ensure_ascii=False)
            yield text

        self._last_stream_answer = self._normalize_rag_answer(answer_dict)
...
class BaseDashscopeProcessor:
    def __init__(self):
        ...
        self.default_model = DEFAULT_ANSWERING_MODEL
        self.last_stream_content = ""
        self.last_parsed_response = None

    @staticmethod
    def _parse_content_to_dict(content: str) -> dict:
        try:
    ...
    def send_message_stream(
        ...
        system_content='You are a helpful assistant.',
        human_content='Hello!',
        max_tokens: int = RAG_ANSWER_MAX_TOKENS,
        **kwargs
    ):
        ...
        for rsp in responses:
            delta = extract_dashscope_stream_delta(rsp)
            if delta:
                chunks.append(delta)
                yield delta
            usage = rsp.get("usage") if isinstance(rsp, dict) else getattr(rsp, "usage", None)
            if usage:
        ...
        self.last_stream_content = "".join(chunks)
        self.last_parsed_response = self._parse_content_to_dict(self.last_stream_content)
    ...
    def send_message(
        ...
        **kwargs
        ...
        #print('='*30)
        # 调用 dashscope（qwen3.7 系列需走多模态接口）
        response = dashscope_chat_call(
            model=model,
            messages=messages,
            temperature=temperature,
            result_format='message',
        ...
        content = extract_dashscope_message_content(response)
        ...
        parsed = self._parse_content_to_dict(content)
        self.last_stream_content = content

# Qwen3.7-Plus API的基础限流设置为每分钟不超过500次API调用（QPM）。同时，Token消耗限流为每分钟不超过500,000 Tokens
...
class RunConfig:
    ...
    parallel_requests: int = 1 # 并行的数量，需要限制，否则qwen3.7-plus会超出阈值
    pipeline_details: str = ""
    submission_file: bool = True
    full_context: bool = False
    api_provider: str = "dashscope" #openai
    answering_model: str = "qwen3.7-plus" # gpt-4o-mini-2024-07-18 or "gpt-4o-2024-08-06"
    config_suffix: str = ""
...
class Pipeline:
    ...
    def export_reports_to_markdown(self, file_name: str, skip_existing: bool = True) -> bool:
        """
        使用 MinerU 将指定 PDF 转换为 markdown，并保存到 reports_markdown_path。
        :param file_name: PDF 文件名（如 '【财报】中芯国际：中芯国际2024年年度报告.pdf'）
        :param skip_existing: 目标 markdown 已存在时是否跳过
        :return: 是否成功（含已存在跳过的情况）
        """
        base_name = os.path.splitext(file_name)[0]
        target_path = self.paths.reports_markdown_path / f"{base_name}.md"

        if skip_existing and target_path.exists():
            print(f"Markdown 已存在，跳过: {target_path}")
            return True

        print(f"开始 MinerU 解析: {file_name}")
        os.makedirs(self.paths.reports_markdown_path, exist_ok=True)

        merged_md_path = pdf_mineru.parse_pdf_by_name(
            file_name,
            local_pdf_dir=self.paths.pdf_reports_dir,
        )
        if not merged_md_path.exists():
            raise FileNotFoundError(f"未找到 MinerU 解析结果: {merged_md_path}")

        shutil.copy2(merged_md_path, target_path)
        print(f"已将 Markdown 保存到: {target_path}")
        return True

    def export_all_reports_to_markdown(self, skip_existing: bool = True):
        """批量解析 pdf_reports 目录下的所有 PDF，单个失败不影响其余文件。"""
        pdf_files = sorted(self.paths.pdf_reports_dir.glob("*.pdf"))
        if not pdf_files:
            print(f"未在 {self.paths.pdf_reports_dir} 找到 PDF 文件")
            return

        print(f"共发现 {len(pdf_files)} 个 PDF，开始使用 MinerU 解析...")
        success_count = 0
        failed_count = 0

        for pdf_path in pdf_files:
            try:
                if self.export_reports_to_markdown(pdf_path.name, skip_existing=skip_existing):
                    success_count += 1
            except Exception as e:
                failed_count += 1
                print(f"[错误] 解析失败，已跳过: {pdf_path.name} -> {e}")
                logging.exception("MinerU 解析失败: %s", pdf_path.name)

        print(
            f"MinerU 批量解析完成: 成功 {success_count}，失败 {failed_count}，合计 {len(pdf_files)}"
        )
    ...
    def chunk_reports(self, include_serialized_tables: bool = False):
        ...
        text_splitter.split_markdown_reports(
            ...
            pdf_dir=self.paths.pdf_reports_dir,
        )
    ...
    def process_questions(self):
        ...
        processor = QuestionsProcessor(
            ...
            rerank_api_key=self.run_config.rerank_api_key,
    ...
    def answer_single_question(self, question: str, kind: str = "string"):
        ...
        self._stream_processor = self._create_questions_processor()
        ...
        t3 = time.time()
        print(f"[计时] process_single_question 推理耗时: {t3-t2:.2f} 秒")
        print(f"[计时] answer_single_question 总耗时: {t3-t0:.2f} 秒")
        return answer

    def _create_questions_processor(self) -> QuestionsProcessor:
    ...
    def answer_single_question_stream(self, question: str, kind: str = "string"):
        """
        单条问题流式推理，逐块 yield LLM 文本。
        流结束后调用 get_last_stream_answer() 获取结构化答案。
        """
        print("[计时] 开始初始化 QuestionsProcessor（流式）...")
        self._stream_processor = self._create_questions_processor()
        yield from self._stream_processor.process_single_question_stream(question, kind=kind)

    def get_last_stream_answer(self) -> dict | None:
        if hasattr(self, "_stream_processor"):
            return self._stream_processor.get_last_stream_answer()
        return None

    def get_last_retrieval_results(self) -> list:
        if hasattr(self, "_stream_processor"):
            return self._stream_processor.get_last_retrieval_results()
        return []

    def get_last_retrieval_elapsed(self) -> float:
        if hasattr(self, "_stream_processor"):
            return self._stream_processor.get_last_retrieval_elapsed()
        return 0.0

    def get_last_rerank_elapsed(self) -> float:
        if hasattr(self, "_stream_processor"):
            return self._stream_processor.get_last_rerank_elapsed()
        return 0.0

    def get_last_rerank_fallback(self) -> bool:
        if hasattr(self, "_stream_processor"):
            return self._stream_processor.get_last_rerank_fallback()
        return False

    def get_last_rerank_fallback_reason(self) -> str | None:
        if hasattr(self, "_stream_processor"):
            return self._stream_processor.get_last_rerank_fallback_reason()
        return None

    def get_last_llm_elapsed(self) -> float:
        if hasattr(self, "_stream_processor"):
            return self._stream_processor.get_last_llm_elapsed()
        return 0.0
...
max_config = RunConfig(
    ...
    parallel_requests=4,
    submission_file=True,
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = qwen3.7-plus",
    answering_model="qwen3.7-plus",
    config_suffix="_qwen3_7_plus"
)
...
if __name__ == "__main__":
    ...
    pipeline.export_all_reports_to_markdown()
